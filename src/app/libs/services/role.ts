...
const mockData: Role[] = ROLE_DATA.map((role, i) => {
  ...
  const createdDate = new Date(currentDate.getTime() - (Math.random() * 1460 * 24 * 60 * 60 * 1000)); 
  ...
  return {
    ...
    status: role.level === 'Executive' || role.level === 'Management' ? 1 : Math.random() > 0.1 ? 1 : 0,
...
// function สำหรับดึงข้อมูล Role สำหรับ SearchField ในหน้า Role เอง
